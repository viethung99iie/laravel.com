            ...
            <div class="perpage">
                ...
                <div class="uk-flex uk-flex-middle uk-flex-space-between ">
                    <select id="records" name="perpage" class="form-control filter mr10">
                        @for($i=20;$i<=200;$i+=20) <option value="{{ $i }}" @selected($perpage==$i)> {{ $i }} {{__('messages.perpage')}}</option>
                ...
                <div class="uk-flex uk-flex-middle ">
                    ...
                    <div class="uk-flex uk-flex-middle  mr10 ">
                        <select name="post_catalogue_id" class="form-control setUpSelect2" id="">
                            ...
                            <option @selected(Request('post_catalogue_id')==$key) value="{{$key}}">{{$val}}
                            </option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <a href="{{route('post.create')}}" class="btn btn-warning"><i class="fa fa-plus mr5">
                            <span>{{$title}}</span></i></a>
                </div>
            </div>
        </div>
